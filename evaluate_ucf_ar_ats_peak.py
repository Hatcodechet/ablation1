...
OUTPUT4_ROOT = Path("/workspace/ablation1/output4")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate UCF-CrimeAR retrieval using a hybrid ATS + anomaly peak sampler."
    )
    ...
    parser.add_argument("--peak-frames", type=int, default=4)
    parser.add_argument("--ats-tau", type=float, default=0.05)
    parser.add_argument("--clip-cache-dir", type=Path, default=Path("/workspace/ablation1/clip_cache"))
...
def hybrid_tag(select_frames: int, peak_frames: int, tau: float) -> str:
    tau_int = int(round(tau * 100))
    return f"ats_peak_f{select_frames}_k{peak_frames}_tau{tau_int:02d}"


def resolve_output_paths(args: argparse.Namespace) -> None:
    suffix = hybrid_tag(args.select_frames, args.peak_frames, args.ats_tau)
    if args.embedding_cache is None:
        args.embedding_cache = OUTPUT4_ROOT / f"{suffix}_embedding_cache.pt"
    if args.metrics_json is None:
        args.metrics_json = OUTPUT4_ROOT / f"{suffix}_metrics.json"
    if args.ranks_json is None:
        args.ranks_json = OUTPUT4_ROOT / f"{suffix}_ranks.json"
...
def hybrid_ats_peak_sample(
    ...
    if peak_frames < 0:
        raise ValueError("--peak-frames must be >= 0.")

    num_frames = int(anomaly_scores.shape[0])
    if num_frames == 0:
        raise ValueError("Hybrid ATS + Peak sampling received an empty anomaly score sequence.")

    select_frames = min(select_frames, num_frames)
    peak_frames = min(peak_frames, select_frames)
    ats_frames = max(0, select_frames - peak_frames)

    selected = set()

    if ats_frames > 0:
        selected.update(ats_density_aware_sample(anomaly_scores, ats_frames, tau))

    if peak_frames > 0:
        peak_indices = torch.argsort(anomaly_scores, descending=True)[:peak_frames].tolist()
        selected.update(int(index) for index in peak_indices)

    if len(selected) < select_frames:
        ranked_indices = torch.argsort(anomaly_scores, descending=True).tolist()
...
def evaluate(args: argparse.Namespace) -> tuple[dict, dict]:
    ...
    def embedding_builder(frame_features: torch.Tensor) -> torch.Tensor:
        ...
        sampled_indices = hybrid_ats_peak_sample(
            anomaly_scores,
